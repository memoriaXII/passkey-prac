...
export function parseAuthenticatorData(
  ...
  if (flags.at) {
    ...
    const badEdDSACBOR = isoUint8Array.fromHex(
    ...
    const firstEncoded = Uint8Array.from(
       ...
       */
      isoCBOR.encode(
        firstDecoded as unknown as Map<number, number | Uint8Array>
      )
    );
  ...
  if (flags.ed) {
    ...
    const firstDecoded = isoCBOR.decodeFirst<AuthenticatorExtensionData>(
      authData.slice(pointer)
    );
